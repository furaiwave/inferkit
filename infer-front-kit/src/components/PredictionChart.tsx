...
export function PredictionChart({ prediction, className }: Props) {
    ...
    switch (result.kind) {
      case 'classification': {
        ...
        return (
          <div className="space-y-4">
            <div>
              <p className="text-xs text-muted-foreground font-medium mb-1">Передбачений клас</p>
              <p className="text-2xl font-mono font-bold">{result.predicted_class}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground font-medium">Ймовірності</p>
              {sorted.map(([cls, prob]) => (
                <div key={cls}>
      ...
      case 'regression':
        return (
          <div className="space-y-3">
            <div>
              <p className="text-xs text-muted-foreground font-medium mb-1">Передбачене значення</p>
            ...
            {result.confidence_interval && (
              <div>
                <p className="text-xs text-muted-foreground font-medium mb-1">95% довірчий інтервал</p>
      ...
      case 'clustering':
        return (
          <div className="space-y-3">
            <div>
              <p className="text-xs text-muted-foreground font-medium mb-1">Кластер</p>
              <p className="text-3xl font-mono font-bold">#{result.cluster_id}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground font-medium mb-1">Відстань до центроїда</p>
  ...
  return (
    <Card className={className}>
      <CardHeader><CardTitle>Результат прогнозу</CardTitle></CardHeader>
      <CardContent>
        {content}
